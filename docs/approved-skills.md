...
| Knowledge Base Query | `/kb-query [query] [--domain D] [--applies-to V] [--top-k N]` | Low–Medium | Runs `knowledge-base/scripts/query.py` to retrieve relevant knowledge base entries via semantic search. Returns ranked results with titles, domains, similarity scores, and entry snippets. Read-only. | **Requires pre-built index** — run `embed.py` first if DB is absent. **Network**: `TRANSFORMERS_OFFLINE=1` is set in query.py — no HuggingFace Hub calls at runtime. Packages: lancedb (Tier 2, APPROVED WITH CONDITIONS — local path only), sentence-transformers (Tier 3, Operator sign-off 2026-03-14). Phase 4 strace confirmed: zero external network connections. | Joshua Alexander Clement | 2026-03-14 |
| Session Start | `/session-start` | Low | Reads project state (progress.md, constitution, STD05, active work docs), checks git branch, and outputs a compact status briefing. Pure-prompt, no file writes, no execution. | Does not begin work until briefing is output and user responds. | Joshua Alexander Clement | 2026-03-15 |
| Session End | `/session-end` | Low–Medium | Enforces STD05 session-end checklist: updates progress.md (session note, active/completed tables), updates MEMORY.md if needed, commits staged files by name with STD03 message, pushes, and reports commit hash. | Writes to `registry/progress.md` and optionally `memory/MEMORY.md`. Runs `git commit` and `git push` — confirm branch before invoking. | Joshua Alexander Clement | 2026-03-15 |

### Spec-Kit Skills (github.com/github/spec-kit)

Skills from GitHub's Spec-Driven Development toolkit. All require spec-kit to be installed in the target project (`scripts/bash/` directory present, feature-branch naming convention active). These commands will **fail safely** at the script-execution step in LLM-Quickstart (spec-kit not installed). Phase 4 execution tests are deferred and required before first use in any spec-kit-enabled project.

> **Shared prerequisite**: All spec-kit skills depend on `check-prerequisites.sh` sourcing `common.sh`. These scripts must be present at `scripts/bash/` relative to the project root.

> **Extension hooks**: `speckit.implement` and `speckit.tasks` read `.specify/extensions.yml` for hook definitions. Mandatory hooks with `optional: false` trigger `EXECUTE_COMMAND: {command}` auto-execution. Review `extensions.yml` before each invocation in any project where it exists.

| Skill | Invocation | Risk | What It Does | Caveats | Approved | Date |
|---|---|---|---|---|---|---|
| Spec Analyze | `/speckit.analyze` | Low | **READ-ONLY** cross-artifact consistency analysis across spec.md, plan.md, tasks.md. 6 detection passes (duplication, ambiguity, underspecification, constitution alignment, coverage gaps, inconsistency). Outputs Markdown report — no file writes. | Requires spec-kit installed + tasks.md to exist. Constitution loaded from `/memory/constitution.md` (path matches LLM-Quickstart). Phase 4 deferred. | Joshua Alexander Clement | 2026-03-14 |
| Spec Checklist | `/speckit.checklist [domain]` | Low–Medium | Generates "unit tests for requirements" checklists — validates requirement quality (completeness, clarity, consistency). Writes/appends to `FEATURE_DIR/checklists/[domain].md`. | Requires spec-kit installed. Write scope bounded to FEATURE_DIR/checklists/. Phase 4 deferred. | Joshua Alexander Clement | 2026-03-14 |
| Spec Clarify | `/speckit.clarify` | Medium | Structured spec clarification: asks up to 5 targeted questions, encodes answers back into spec.md (atomic overwrites after each answer). | Requires spec-kit installed. Modifies spec.md — ensure clean git state before invoking. Max 5 questions hard-capped. Phase 4 deferred. | Joshua Alexander Clement | 2026-03-14 |
| Spec Constitution | `/speckit.constitution` | Medium | Creates/updates project constitution at `.specify/memory/constitution.md`. Updates version, propagates changes across spec-kit templates. | **NOT for use in LLM-Quickstart** — path mismatch: operates on `.specify/memory/constitution.md`, not `memory/constitution.md`. For fresh spec-kit projects only. Human review of Sync Impact Report required after update. | Joshua Alexander Clement | 2026-03-14 |
| Spec Implement | `/speckit.implement` | High | Full implementation execution: processes all tasks.md tasks phase-by-phase, creates ignore files, checks checklists before proceeding. | **Review `.specify/extensions.yml` before each use** — mandatory hooks auto-execute `EXECUTE_COMMAND` without confirmation. Requires spec-kit installed + tasks.md. Clean git state before invoking. Phase 4 deferred. | Joshua Alexander Clement | 2026-03-14 |
| Spec Plan | `/speckit.plan` | Medium | Generates design artifacts: research.md, data-model.md, contracts/, quickstart.md. Runs `setup-plan.sh` and `update-agent-context.sh` (can modify CLAUDE.md). | **Review `update-agent-context.sh` source before first use** — verifies it cannot overwrite AGENTS.md/CLAUDE.md unconditionally. Requires spec-kit + spec.md. Phase 4 deferred (both scripts). | Joshua Alexander Clement | 2026-03-14 |
| Spec Specify | `/speckit.specify [description]` | Medium | Creates feature git branch (`specify/N-name`), writes spec.md, generates checklists/requirements.md. | **Creates and switches to new git branch** — ensure clean working tree. Requires spec-kit installed. Run once per feature. Phase 4 deferred. | Joshua Alexander Clement | 2026-03-14 |
| Spec Tasks | `/speckit.tasks` | Medium | Generates tasks.md organized by user story with dependency ordering, parallel markers, and phase structure. | **Review `.specify/extensions.yml` before use** if present — same extension hook concern as `speckit.implement`. Requires spec-kit + plan.md. Phase 4 deferred. | Joshua Alexander Clement | 2026-03-14 |
| Spec Tasks to Issues | `/speckit.taskstoissues` | High | Converts tasks.md entries into GitHub issues using `github/github-mcp-server/issue_write`. | **External, visible, not easily reversible** — issues created on GitHub repo. **Require user confirmation of issue count before execution.** `github/github-mcp-server` MCP must be configured. Remote URL must match intended repo. Phase 4 test required in disposable repo first. | Joshua Alexander Clement | 2026-03-14 |
