...
| Skill | Invocation | Risk | What It Does | Caveats | Approved | Date |
|---|---|---|---|---|---|---|
| Playwright Pro | `/pw:init` `/pw:generate` `/pw:review` `/pw:fix` `/pw:migrate` `/pw:coverage` `/pw:report` | Low–Medium | Production Playwright testing toolkit. Generates, reviews, fixes, and migrates E2E tests. 55 templates, 3 agents (test-architect, test-debugger, migration-planner). Auto-lints test files on every Write/Edit via hook. | **Hooks auto-fire**: `validate-test.sh` runs on every Write/Edit to spec files (read-only linter, prints warnings). `detect-playwright.sh` runs at session start (read-only). **Pre-granted**: `Bash(npx playwright*)` and `Bash(npx tsx*)` — Claude can run any Playwright CLI command without per-use confirmation. `/pw:testrail` and `/pw:browserstack` require env vars set by user; MCP integrations not yet reviewed — use only if MCPs are explicitly configured. | Joshua Alexander Clement | 2026-03-13 |
...
| 1.8 | 2026-03-13 | Joshua Alexander Clement | claude-sonnet-4-6 | Approved playwright-pro — moved to §2 Testing Skills. Auto-firing lint hook and pre-granted Playwright CLI permissions accepted by Operator as intentional design for a testing tool. MCP integrations (TestRail, BrowserStack) remain unreviewed — do not use /pw:testrail or /pw:browserstack until reviewed. |
