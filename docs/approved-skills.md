...
| `engineering/skill-tester` | alirezarezvani/claude-skills | Phase 4 pending | — | 2 stdlib Python tools: script_tester.py (executes Python scripts with timeout), skill_validator.py, quality_scorer.py. PyYAML guarded. Executes Python scripts — must verify `/tmp/` sandbox enforcement under strace before approval. |
